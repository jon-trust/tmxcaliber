...
class FilterApplier:
    def __init__(self, filter: Filter, exclude_as_filter: bool) -> None:
        self.filter = filter
        self.exclude_as_filter = exclude_as_filter

    def apply_filter(self, threatmodel_data: ThreatModelData) -> None:
    ...
    def __filter_by_severity(self, threatmodel_data: ThreatModelData) -> None:
    ...
    def __filter_controls_by_current_threats(
    ...
    def __filter_control_objectives_by_current_controls(
        ...
        for co_id in threatmodel_data.control_objectives.copy():
    ...
    def __filter_by_feature_class(self, threatmodel_data: ThreatModelData) -> None:
        if self.exclude_as_filter:
            ...
            feature_class_ids_to_keep: list[str] = []
    ...
    def __filter_relationships_by_current_feature_classes(
        ...
        for feature_class in threatmodel_data.feature_classes.values():
            relations_to_keep = [
                relation
    ...
    def __filter_controls_by_current_feature_classes(
        ...
        for control_id, control in threatmodel_data.controls.copy().items():
            for feature_class in (control.get("feature_class") or []).copy():
                if feature_class not in threatmodel_data.feature_classes:
                    threatmodel_data.controls[control_id]["feature_class"].remove(
                        feature_class
                    )

    def __filter_by_permissions(self, threatmodel_data: ThreatModelData) -> None:
        ...
        for threat_id, threat in threatmodel_data.threats.copy().items():
            ...
            # Apply the exclusion or inclusion logic based on exclude_as_filter
            if self.exclude_as_filter:
                if has_access:
                    threatmodel_data.threats.pop(threat_id)
            elif not has_access:
    ...
    def __filter_by_threats(self, threatmodel_data: ThreatModelData) -> None:
        if self.exclude_as_filter:
    ...
    def __pop_downstream_dependent_controls(
        self, controls_to_filter: list[str], threatmodel_data: ThreatModelData
    ) -> None:
    ...
    def __pop_upstream_dependent_controls(
        ...
        all_control_dependencies: dict[str, object] = {}
    ...
    def __filter_orphan_assurance_controls(
        self, threatmodel_data: ThreatModelData
    ) -> None:
        active_assurance_control_ids: list[str] = []
        for control in threatmodel_data.controls.values():
        ...
        for control_id, control in threatmodel_data.controls.copy().items():
            ...
            if control["coso"] != "Assurance":
                ...
                filtered_ids = [
                    assurance_id
                    for assurance_id in control["assured_by"].split(",")
                    if assurance_id in threatmodel_data.controls
                ]
    ...
    def __filter_by_controls(
        self, filter_control_ids: list[str], threatmodel_data: ThreatModelData
    ) -> None:
        if self.exclude_as_filter:
    ...
    def __filter_by_control_objectives(self, threatmodel_data: ThreatModelData) -> None:
